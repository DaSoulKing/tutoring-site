...
router.get('/dashboard', isAuthenticated, isParent, async (req, res) => {
    try {
        ...
        const subscription = await pool.query(`
            SELECT * FROM subscriptions WHERE parent_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1
        `, [userId]);
        ...
        const reportCards = await pool.query(`
            ...
            JOIN users t ON rc.tutor_id = t.id
            WHERE rc.student_id = $1
            ORDER BY rc.report_date DESC LIMIT 5
        `, [userId]);

        res.render('parent/dashboard', {
            title: 'My Dashboard - BrightMinds',
...
router.get('/calendar', isAuthenticated, isParent, async (req, res) => {
    try {
        ...
        res.render('parent/calendar', {
            title: 'My Calendar - BrightMinds',
            bookings: bookings.rows,
            meta: {}
        });
    } catch (err) {
...
// Messages
router.get('/messages', isAuthenticated, isParent, async (req, res) => {
    try {
        // Get conversations
...
router.get('/messages/:userId', isAuthenticated, isParent, async (req, res) => {
    try {
        ...
        res.render('parent/messages', {
            title: `Chat with ${otherUser.rows[0]?.first_name || 'User'} - BrightMinds`,
            conversations: conversations.rows,
            activeConversation: otherUser.rows[0] || null,
...
// Send message
router.post('/messages/:userId', isAuthenticated, isParent, async (req, res) => {
    try {
        const { body } = req.body;
        await pool.query(`
            INSERT INTO messages (sender_id, receiver_id, body, message_type)
            VALUES ($1, $2, $3, 'general')
        `, [req.session.user.id, req.params.userId, body]);
...
router.get('/contact', isAuthenticated, isParent, (req, res) => {
    res.render('parent/contact-form', {
        title: 'Contact & Inquiries - BrightMinds',
        meta: {}
    });
});
...
router.post('/contact', isAuthenticated, isParent, async (req, res) => {
    try {
        ...
        await pool.query(`
            INSERT INTO inquiries (user_id, name, email, inquiry_type, subject, message)
            VALUES ($1, $2, $3, $4, $5, $6)
        `, [user.id, `${user.firstName} ${user.lastName}`, user.email, inquiry_type, subject, message]);

        req.session.success = 'Your inquiry has been submitted. We will get back to you soon!';
        res.redirect('/parent/contact');
    } catch (err) {
        console.error(err);
        req.session.error = 'Failed to submit inquiry.';
        res.redirect('/parent/contact');
    }
});
...
router.post('/cancel-subscription', isAuthenticated, isParent, async (req, res) => {
    try {
        ...
        `, [cancel_reason, req.session.user.id]);

        req.session.success = 'Your subscription has been cancelled.';
        res.redirect('/parent/dashboard');
    } catch (err) {
        console.error(err);
        req.session.error = 'Failed to cancel subscription.';
        res.redirect('/parent/dashboard');
    }
});
...
router.get('/session/:bookingId', isAuthenticated, async (req, res) => {
    try {
        ...
        // Save room ID if not set
        if (!b.meeting_room_id) {
            await pool.query('UPDATE bookings SET meeting_room_id = $1 WHERE id = $2', [roomId, b.id]);
        }

        res.render('parent/video-session', {
            title: 'Video Session - BrightMinds',
            booking: b,
            roomId: roomId,
