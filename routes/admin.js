...
router.get('/owner/match', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        const tutors = await pool.query(`SELECT u.id, u.first_name, u.last_name, tp.subjects, tp.hourly_rate, tp.grade_levels, tp.subject_grades FROM users u JOIN tutor_profiles tp ON u.id = tp.user_id WHERE u.role = 'tutor' AND u.is_active = true AND tp.approved = true ORDER BY u.first_name`);
