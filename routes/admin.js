...
router.get('/owner', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        const checkins = await pool.query(`
            ...
            FROM checkins c JOIN users u ON c.student_id = u.id
            WHERE c.completed = false AND c.due_date <= CURRENT_DATE + INTERVAL '7 days'
            ORDER BY c.due_date LIMIT 20
        `);

        const payments = await pool.query(`
            SELECT s.*, u.first_name, u.last_name, u.email, u.phone
            FROM subscriptions s JOIN users u ON s.parent_id = u.id
            WHERE s.status = 'active' AND s.next_billing_date <= CURRENT_DATE + INTERVAL '7 days'
            ORDER BY s.next_billing_date LIMIT 20
        `);
        ...
        const pendingTutors = await pool.query(`
            SELECT u.*, tp.subjects, tp.bio FROM users u
            JOIN tutor_profiles tp ON u.id = tp.user_id
            WHERE u.role = 'tutor' AND tp.approved = false AND u.is_active = true
        `);

        res.render('admin/owner-dashboard', {
            title: 'Owner Dashboard - BrightMinds', stats, checkins: checkins.rows,
...
router.post('/owner/tutor-invite', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        const subjectsArray = subjects_for_invite ? subjects_for_invite.split(',').map(s => s.trim()) : [];

        await pool.query(`
            INSERT INTO tutor_invites (token, email, subjects, created_by, expires_at)
            VALUES ($1, $2, $3, $4, $5)
        `, [token, email || null, subjectsArray, req.session.user.id, expires]);

        const inviteUrl = `${process.env.SITE_URL || 'http://localhost:3000'}/tutor-signup/${token}`;
        ...
        try {
            ...
            if (process.env.SMTP_HOST && email) {
                ...
                await transporter.sendMail({
                    ...
                    subject: 'You are invited to join BrightMinds Tutoring!',
                    html: `<h2>Welcome to BrightMinds!</h2><p>You have been invited to join as a tutor. Click the link below to create your account:</p><p><a href="${inviteUrl}">${inviteUrl}</a></p><p>This link expires in 7 days.</p>`
                });
            }
        } catch (emailErr) { console.error('Email send failed:', emailErr); }

        req.session.success = `Invite link generated: ${inviteUrl}`;
...
router.post('/owner/referral-code', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        await pool.query(`
            INSERT INTO referral_codes (code, discount_percent, created_by, note, is_active)
            VALUES ($1, $2, $3, $4, true)
        ...
        res.redirect('/admin/owner');
    } catch (err) { console.error(err); req.session.error = 'Failed to create code.'; res.redirect('/admin/owner'); }
});
...
router.post('/owner/assign-tutor', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        // Verify both exist
        const tutor = await pool.query("SELECT id FROM users WHERE id = $1 AND role = 'tutor'", [tutor_id]);
        const student = await pool.query("SELECT id FROM users WHERE id = $1 AND role IN ('parent','student')", [student_id]);

        if (tutor.rows.length === 0 || student.rows.length === 0) {
            req.session.error = 'Invalid tutor or student.';
            return res.redirect('/admin/owner/students');
        }

        // Create a pending booking as assignment placeholder
        await pool.query(`
            INSERT INTO bookings (tutor_id, student_id, parent_id, booking_date, start_time, end_time, subject, status)
            VALUES ($1, $2, $2, CURRENT_DATE, '00:00', '00:00', 'Assigned by Admin', 'confirmed')
        `, [tutor_id, student_id]);

        // Update checkins to link tutor
        await pool.query('UPDATE checkins SET tutor_id = $1 WHERE student_id = $2 AND completed = false', [tutor_id, student_id]);

        req.session.success = 'Tutor assigned to student!';
        res.redirect('/admin/owner/students');
    } catch (err) { console.error(err); req.session.error = 'Failed to assign.'; res.redirect('/admin/owner/students'); }
});
...
router.get('/owner/tutors', isAuthenticated, isOwner, async (req, res) => {
    try {
        const tutors = await pool.query(`
            SELECT u.*, tp.subjects, tp.approved, tp.bio, tp.experience_years, tp.tagline, tp.education
            FROM users u JOIN tutor_profiles tp ON u.id = tp.user_id WHERE u.role = 'tutor' ORDER BY u.first_name
        `);
...
// Manage students with tutor assignment
router.get('/owner/students', isAuthenticated, isOwner, async (req, res) => {
    try {
        const students = await pool.query(`
            ...
            WHERE u.role IN ('parent', 'student') ORDER BY u.first_name
        `);
        const tutors = await pool.query(`
            SELECT u.id, u.first_name, u.last_name FROM users u
            JOIN tutor_profiles tp ON u.id = tp.user_id
            WHERE u.role = 'tutor' AND u.is_active = true AND tp.approved = true ORDER BY u.first_name
        `);
...
router.get('/owner/applications/:id', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        if (app.rows.length === 0) { req.session.error = 'Application not found.'; return res.redirect('/admin/owner'); }
...
router.get('/owner/inquiries/:id', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        if (inq.rows.length === 0) { req.session.error = 'Inquiry not found.'; return res.redirect('/admin/owner'); }
...
router.post('/owner/checkins/:id/complete', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE checkins SET completed = true, completed_at = NOW(), notes = $1 WHERE id = $2`, [notes, req.params.id]);
        ...
        await pool.query(`INSERT INTO checkins (student_id, tutor_id, parent_id, due_date) VALUES ($1, $2, $3, CURRENT_DATE + INTERVAL '3 months')`, [c.student_id, c.tutor_id, c.parent_id]);
        req.session.success = 'Check-in completed. Next one scheduled.';
        res.redirect('/admin/owner');
    } catch (err) { console.error(err); res.redirect('/admin/owner'); }
});
...
router.get('/owner/notes/:userId', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        const notes = await pool.query(`
            ...
            FROM notes n JOIN users u ON n.author_id = u.id WHERE n.target_user_id = $1
...
router.post('/owner/notes/:userId', isAuthenticated, isOwner, async (req, res) => {
    ...
    } catch (err) { console.error(err); res.redirect(`/admin/owner/notes/${req.params.userId}`); }
});
...
router.get('/owner/calendar', isAuthenticated, isOwner, async (req, res) => {
    try {
        const bookings = await pool.query(`
            ...
            WHERE b.booking_date >= CURRENT_DATE - INTERVAL '30 days' ORDER BY b.booking_date, b.start_time
        `);
...
router.get('/owner/blog/new', isAuthenticated, isOwner, (req, res) => {
    res.render('admin/blog-edit', { title: 'New Blog Post', post: null, meta: {} });
});
...
router.post('/owner/blog', isAuthenticated, isOwner, async (req, res) => {
    ...
    } catch (err) { console.error(err); req.session.error = 'Failed to create post.'; res.redirect('/admin/owner/blog'); }
});
...
router.get('/owner/blog/:id/edit', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        res.render('admin/blog-edit', { title: 'Edit Blog Post', post: post.rows[0], meta: {} });
...
router.post('/owner/blog/:id', isAuthenticated, isOwner, async (req, res) => {
    try {
        ...
        const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        await pool.query(`
            UPDATE blog_posts SET title=$1, slug=$2, excerpt=$3, content=$4, is_published=$5,
        ...
        req.session.success = 'Blog post updated!';
        res.redirect('/admin/owner/blog');
    } catch (err) { console.error(err); req.session.error = 'Failed to update.'; res.redirect('/admin/owner/blog'); }
});
...
router.get('/owner/referrals', isAuthenticated, isOwner, async (req, res) => {
    try {
        const codes = await pool.query(`
            SELECT rc.*, u.first_name as used_first, u.last_name as used_last
            FROM referral_codes rc LEFT JOIN users u ON rc.used_by = u.id
...
router.get('/tutor', isAuthenticated, isTutor, async (req, res) => {
    try {
        ...
        const sessions = await pool.query(`
            ...
            AND b.subject != 'Assigned by Admin'
        ...
        res.render('admin/tutor-dashboard', {
            ...
            availability: availability.rows, meta: {}
        });
    } catch (err) { console.error(err); req.session.error = 'Failed to load dashboard.'; res.redirect('/'); }
});
...
router.post('/tutor/profile', isAuthenticated, isTutor, upload.single('profile_picture'), async (req, res) => {
    try {
        ...
        if (req.file) {
            await pool.query('UPDATE users SET profile_picture = $1 WHERE id = $2', ['/uploads/' + req.file.filename, req.session.user.id]);
        }
    ...
    } catch (err) { console.error(err); req.session.error = 'Failed to update.'; res.redirect('/admin/tutor'); }
});
...
router.post('/tutor/availability', isAuthenticated, isTutor, async (req, res) => {
    ...
    } catch (err) { console.error(err); res.json({ success: false, message: 'Failed to update.' }); }
});
...
router.get('/tutor/session-sheet/:bookingId', isAuthenticated, isTutor, async (req, res) => {
    try {
        const booking = await pool.query(`
            SELECT b.*, s.first_name as student_first, s.last_name as student_last
            FROM bookings b JOIN users s ON b.student_id = s.id WHERE b.id = $1 AND b.tutor_id = $2
...
router.post('/tutor/session-sheet/:bookingId', isAuthenticated, isTutor, async (req, res) => {
    try {
        ...
        await pool.query(`
            ...
            ON CONFLICT (booking_id) DO UPDATE SET topics_covered=$4, homework_assigned=$5, student_performance=$6, notes=$7, next_session_plan=$8
        ...
        res.redirect('/admin/tutor');
    } catch (err) { console.error(err); req.session.error = 'Failed to save.'; res.redirect('/admin/tutor'); }
});
...
router.post('/tutor/report-card/:studentId', isAuthenticated, isTutor, async (req, res) => {
    try {
        ...
        await pool.query(`
            INSERT INTO report_cards (tutor_id, student_id, term, report_date, overall_grade, subjects, attendance_score, participation_score, homework_score, comments, strengths, areas_for_improvement, goals_next_term)
    ...
    } catch (err) { console.error(err); req.session.error = 'Failed to create.'; res.redirect('/admin/tutor'); }
});
...
router.post('/tutor/notes/:studentId', isAuthenticated, isTutor, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO notes (author_id, target_user_id, target_type, content) VALUES ($1,$2,'student',$3)`, [req.session.user.id, req.params.studentId, content]);
        req.session.success = 'Note added.';
        res.redirect('/admin/tutor');
    } catch (err) { console.error(err); res.redirect('/admin/tutor'); }
});
