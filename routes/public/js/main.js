...
function renderCalendar(state, calEl) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dateStr = state.year + '-' + monthStr + '-' + dayStr;
        const dayEvents = (state.events || []).filter(function(e) {
            // Parse date from string directly to avoid timezone shift
            var bd = (e.booking_date || '').substring(0, 10);
            return bd === dateStr;
        });
...
function initAvailabilityEditor() {
    ...
    editor.addEventListener('click', function(e) {
        ...
        if (target.classList.contains('avail-add')) {
            ...
            // Remainders under 60 min are dropped (no partial slots)
